# Figma Make Prompt برای تولید صفحه فرود

شما باید یک صفحه فرود React کاملاً responsive بر اساس طراحی‌هایی که ارائه می‌دهم تولید کنید. من طراحی‌ها را بخش به بخش برای شما ارسال می‌کنم و شما باید کد تولید شده را به صورت تدریجی به‌روزرسانی کنید.
...
**مهم**: این پاسخ API فقط برای درک ساختار داده‌ها و تولید TypeScript types ارائه شده است. از آن به عنوان mock data در کد استفاده نکنید. داده‌های واقعی در زمان اجرا به عنوان props به component فرود ارسال خواهد شد.

## الزامات اصلی

### زبان و جهت

- تمام صفحات را در جهت RTL (راست به چپ) پیاده‌سازی کنید
- زبان document را روی `fa-IR` تنظیم کنید
- تمام accessibility labels باید به فارسی باشند
- **مهم**: آیکون‌ها یا assets را mirror نکنید - علی‌رغم جهت RTL آن‌ها را بدون تغییر نگه دارید

### Technology Stack

- از React با TypeScript استفاده کنید
- از CSS Modules برای styling استفاده کنید
- از DOMPurify برای HTML sanitization استفاده کنید
- طراحی کاملاً responsive برای موبایل، تبلت و دسکتاپ پیاده‌سازی کنید
- **از واحد `rem` برای تمام اندازه‌گیری‌های طول استفاده کنید** (هرگز از `px` استفاده نکنید)
- **از نام‌های قابل فهم برای تمام assets استفاده کنید** (از hash های سیستماتیک برای تجربه بهتر توسعه اجتناب کنید)
- **تصاویر CMS را در پوشه assets ذخیره نکنید** - تصاویری که از پاسخ API می‌آیند باید از طریق URL ارجاع داده شوند، نه به صورت محلی ذخیره شوند

### Design Tokens

از سیستم design tokens زیر که در آدرس <https://tap30.github.io/web-components/theme> موجود است استفاده کنید:
...
TypeScript types را بر اساس ساختار پاسخ API ارائه شده در بالا تولید کنید. types باید:

1. دقیقاً با پاسخ API مطابقت داشته باشد - هیچ فیلدی را اضافه، حذف یا تغییر ندهید
2. از نام‌های فیلد و ساختار واقعی پاسخ API استفاده کند
3. تمام بخش‌های تعریف شده در پاسخ را پشتیبانی کند

**قبل از شروع تولید هر کدی**، شما باید:

1. API endpoint ارائه شده توسط من را فراخوانی کنید
2. پاسخ کامل API را به من نشان دهید
3. تأیید کنید که ساختار داده‌ها را درک کرده‌اید

این اطمینان می‌دهد که قبل از تولید هر کدی، ما در مورد فرمت دقیق پاسخ API هماهنگ هستیم.

## راه‌اندازی اولیه

قبل از تولید UI components، شما باید:

1. **CSS tokens را به فایل CSS سراسری اضافه کنید** - تمام design tokens ارائه شده در بالا را در یک فایل CSS سراسری (مثل `globals.css` یا `tokens.css`) قرار دهید

2. **TypeScript types تولید کنید** - تعاریف type را بر اساس ساختار پاسخ API در فایل `types.ts` ایجاد کنید. مثال:
...
// هر بخش را بر اساس پاسخ API تعریف کنید
...
کد را با پیروی از این ساختار تولید کنید:
...
type Props = {
  data: LandingData; // Type بر اساس پاسخ API
};
...
landing component باید به این شکل استفاده شود:
...
این طراحی یکپارچگی آسان SSG (Static Site Generation) را فراهم می‌کند.

## Section Components

- هر بخش را به components جداگانه تقسیم کنید
- هر section component داده‌های خود را به عنوان props دریافت می‌کند
- تمام section components را از `components/index.ts` export کنید
- از CSS Modules برای styling هر component استفاده کنید

## مدیریت محتوای HTML

برخی از فیلدهای API حاوی رشته‌های HTML هستند (مثل `"<p>با توجه به آنکه ترافیک نقش موثری در هزینه پیک موتوری دارد...</p>\n"`).

هنگام رندر کردن محتوای HTML:

1. **Sanitize کنید** با استفاده از DOMPurify: `DOMPurify.sanitize(htmlString)`
2. **از مشکلات nesting اجتناب کنید**: `<p>` را داخل تگ‌های `<p>` قرار ندهید
3. از `dangerouslySetInnerHTML` فقط پس از sanitization استفاده کنید
...
صفحه ممکن است پارامترهای UTM را در URL دریافت کند (مثل `?utm_content=buffercf3b2&utm_medium=social&utm_source=snapchat.com&utm_campaign=buffer`).

برای تمام CTAها (دکمه‌ها و لینک‌ها):

1. پارامترهای UTM را از URL فعلی استخراج کنید
2. پارامترهای UTM را sanitize کنید
3. آن‌ها را به تمام URL های CTA برای ردیابی بازاریابی اضافه کنید
...
- از عناصر HTML معنایی استفاده کنید (`<header>`, `<nav>`, `<main>`, `<section>`, `<article>`, `<footer>`, و غیره)
- ARIA labels را در صورت نیاز به فارسی اضافه کنید
- اطمینان حاصل کنید که navigation با کیبورد به درستی کار می‌کند
- alt text برای تمام تصاویر فراهم کنید
- سلسله مراتب صحیح heading را حفظ کنید (h1, h2, h3, و غیره)
- `lang="fa-IR"` را روی document تنظیم کنید
- اطمینان حاصل کنید که کنتراست رنگ استانداردهای WCAG را رعایت می‌کند
- صفحه را کاملاً قابل استفاده توسط screen readers کنید

## طراحی Responsive

من دو طراحی برای هر بخش ارائه خواهم داد:
...
- layout های کاملاً responsive ایجاد کنید که روی موبایل، تبلت و دسکتاپ کار کنند
- از CSS media queries یا تکنیک‌های مدرن CSS (flexbox, grid) استفاده کنید
- اطمینان حاصل کنید که انتقال‌های نرم بین breakpoint ها وجود دارد
- تست کنید که تمام عناصر تعاملی روی دستگاه‌های لمسی کار می‌کنند

## به‌روزرسانی‌های تدریجی

همانطور که طراحی‌ها را بخش به بخش برای شما ارسال می‌کنم:

1. section components جدید اضافه کنید
2. main landing component را برای شامل کردن بخش‌های جدید به‌روزرسانی کنید
3. TypeScript types را در صورت تغییر ساختار API به‌روزرسانی کنید
4. سازگاری با سبک و ساختار کد موجود را حفظ کنید

## کیفیت کد

- کد TypeScript تمیز و قابل نگهداری بنویسید
- از نام‌های معنادار برای متغیرها و components استفاده کنید
- بهترین شیوه‌های React را دنبال کنید
- برای عملکرد بهینه‌سازی کنید
- components را متمرکز و تک‌منظوره نگه دارید
- از TypeScript typing مناسب در سراسر کد استفاده کنید

## ساختار فایل

کد را به شکل زیر سازماندهی کنید:

```txt
/
├── XLanding.tsx          # Main landing component
├── XLanding.module.css   # Main landing styles
├── types.ts              # TypeScript types برای داده‌های API
├── utils/
│   ├── sanitize.ts       # HTML sanitization utilities
│   └── utm.ts            # UTM parameter utilities
└── components/
    ├── index.ts          # Export تمام section components
    ├── HeroSection/
    │   ├── HeroSection.tsx
...
- جهت RTL برای تمام layout ها
- آیکون‌ها/assets را mirror نکنید
- زبان فارسی برای a11y labels
- ساختار دقیق پاسخ API برای types
- محتوای HTML را Sanitize کنید
- پارامترهای UTM را به تمام CTAها اضافه کنید
- طراحی کاملاً responsive
- HTML معنایی و قابل دسترس
